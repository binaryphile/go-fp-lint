...
// TestAnalyzer runs the nine testdata packages: a (compliant, reachable
...
// type outside the permitted set, slice/evil.Mapper), h (chain call inside
// an invoked closure but AFTER an unconditional return -- dead code one
// level deeper), i (compliant: chain call inside a PARENTHESIZED
// immediately-invoked closure). analysistest resolves each package's
// `// want` annotations (or absence) against the reported diagnostics.
func TestAnalyzer(t *testing.T) {
	testdata := analysistest.TestData()
	analysistest.Run(t, testdata, contractcheck.Analyzer, "a", "b", "c", "d", "e", "f", "g", "h", "i")
}
